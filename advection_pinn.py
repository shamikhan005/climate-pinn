...
def analytical_solution(x, t, alpha=ALPHA, c=C, L=L):
    ...
    term1 = np.exp(-(alpha * (np.pi**2) * t) / (L**2) -
                   (c * (x - c * t)) / (2 * alpha))
    term2 = np.sinh((c * L) / (2 * alpha)) / np.sinh(
        (c * x) / (2 * alpha) + (c * L * (L - x + c * t)) / (2 * alpha * L)
    )
    k = 2 * np.pi / L
...
for epoch in range(epochs):
    ...
    du_dx_left = torch.autograd.grad(
        u_bc_left_pred, x_bc_left, torch.ones_like(u_bc_left_pred), create_graph=True
    )[0]
    du_dx_right = torch.autograd.grad(
        u_bc_right_pred, x_bc_right, torch.ones_like(u_bc_right_pred), create_graph=True
    )[0]
    ...
    if (epoch + 1) % 1000 == 0:
        print(
            f"epoch [{epoch + 1}/{epochs}], total loss: {loss.item():.4e}, data: {
...
plt.subplot(1, 3, 1)
plt.pcolormesh(
    t_grid, x_grid, u_pinn_pred, shading="auto", cmap="seismic", vmin=v_min, vmax=v_max
)
...
plt.subplot(1, 3, 2)
plt.pcolormesh(
    t_grid, x_grid, u_analytical, shading="auto", cmap="seismic", vmin=v_min, vmax=v_max
)
plt.colorbar(label="Value u(x,t)")
plt.xlabel("Time (t)")
plt.ylabel("Position (x)")
