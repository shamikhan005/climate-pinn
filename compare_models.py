...
u_ic_sparse = torch.tensor(analytical_solution(x_ic_sparse.cpu().numpy(), t_ic_sparse.cpu().numpy()), dtype=torch.float32, device=device)
...
for epoch in range(EPOCHS):
    ...
    if (epoch + 1) % 2000 == 0:
        print(f"PINN epoch [{epoch+1}/{EPOCHS}], total loss: {loss.item():.4e}, data: {loss_data.item():.4e}, physics: {loss_physics.item():.4e}")

end_time_pinn = time.time()
print(f"PINN training finished in {end_time_pinn - start_time_pinn:.2f} seconds")
...
print(f"data-only NN training finished in {end_time_nn - start_time_nn:.2f} seconds")
...
# --- PINN ---
plt.subplot(2, 3, 1)
plt.pcolormesh(t_grid, x_grid, u_pinn_pred, shading='auto', cmap='Reds', vmin=0, vmax=1)
...
plt.subplot(2, 3, 2)
plt.pcolormesh(t_grid, x_grid, u_analytical, shading='auto', cmap='Reds', vmin=0, vmax=1)
plt.colorbar(label='Temp u(x,t)')
plt.xlabel('Time (t)')
plt.ylabel('Position (x)')
...
plt.subplot(2, 3, 3)
plt.pcolormesh(t_grid, x_grid, error_pinn, shading='auto', cmap='bwr', vmin=-0.1, vmax=0.1)
...
# --- data-only NN ---
plt.subplot(2, 3, 4)
plt.pcolormesh(t_grid, x_grid, u_nn_pred, shading='auto', cmap='Reds', vmin=0, vmax=1)
plt.colorbar(label='Temp u(x,t)')
plt.xlabel('Time (t)')
plt.ylabel('Position (x)')
...
plt.scatter(t_sparse_data.cpu().numpy(), x_sparse_data.cpu().numpy(), 
            s=5, c='blue', marker='x', label=f'{N_SPARSE*3} data points')
plt.legend()
...
plt.subplot(2, 3, 6)
plt.pcolormesh(t_grid, x_grid, error_nn, shading='auto', cmap='bwr', vmin=-0.1, vmax=0.1)
plt.colorbar(label='Absolute Error')
plt.xlabel('Time (t)')
plt.ylabel('Position (x)')
